...
variable "docker_image" {
  description = <<EOF
  Docker image to pull that contains dev tools & dependencies
  EOF
  default     = "mcr.microsoft.com/vscode/devcontainers/ubuntu"
  validation {
    condition = contains([
      "mcr.microsoft.com/vscode/devcontainers/ubuntu",
      ...
      "mcr.microsoft.com/vscode/devcontainers/python-3-anaconda"
    ], var.docker_image)
    error_message = "Invalid memory!"
  }
}
...
resource "coder_agent" "main" {
  ...
  startup_script = <<EOT
    ...
    ${var.dotfiles_uri != "" ? "coder dotfiles -y ${var.dotfiles_uri}" : "echo \"No dotfiles repo specified\""}
  EOT
}
