...
This templates uses [Kubernetes limits and requests](https://kubernetes.io/docs/concepts/configuration/manage-resources-containers/#requests-and-limits) to save space on a cluster, packing more workspaces per node. When a developer runs an intensive task (such as a compile, build), the workspace will still leverage the full resources allocated.
